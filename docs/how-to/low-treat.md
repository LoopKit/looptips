...
Another common question for new OS-AID users is whether to enter the carbohydrates used to treat a low blood sugar. The short answer is that there is not a great agreement on how to deal with low treatment entries, and **both approaches are perfectly valid** - logging your rescue carbs is just as acceptable as leaving them out. The decision is dependent, often, on factors not just about the OS-AID algorithm; factors such as extra effort, use of OpenAPS's autotune (which needs all carbohydrate inputs to do a better estimation), or desire to have more thorough clinical records.
...
* You will normally treat the low with fast-acting carbs first and log it afterward, so your glucose may already be rebounding by the time you open Loop. The entry then has to be **backdated** to when you actually ate; if you forget, Loop assumes the carbs are still to be absorbed, over-predicts the coming rise, and can recommend or deliver too much insulin - setting off the very rebound you were trying to avoid.
...
1. **While treating the low glucose**, try setting a temporary [override target](overrides.md) higher than your normal target for an hour to help keep Loop from aggressively treating a rebound.

2. **Consider adjusting insulin needs in your override.** When changing the insulin needs you are decreasing the effective scheduled basal rate and increasing (making weaker) the insulin sensitivity factor and carb ratio. See [Low Override](overrides.md#low-override).
